...
import type { BranchContextMode } from '../../lib/branchContext';

const empty: CajaPayload = { nombre: '', codigo: '', descripcion: '' };

export default function CajaManager({ api, sucursales = [], sucursalActivaId, contextMode = 'specific', contextVersion = 0 }: { api: CajaApi; sucursales?: Sucursal[]; sucursalActivaId?: number; contextMode?: BranchContextMode; contextVersion?: number }) {
  const platform = sucursales.length > 0;
  const readOnlyConsolidated = !platform && contextMode === 'consolidated';
  ...
  const load = useCallback(async () => {
    ...
    try {
      const nextRows = await api.listar(platform ? filter : undefined);
      if (sequence === requestSequence.current) setRows(nextRows);
    }
    catch (e) { if (sequence === requestSequence.current) setError(cajaError(e, 'No fue posible cargar las cajas.')); }
    finally { if (sequence === requestSequence.current) setLoading(false); }
  }, [api, filter, platform, sucursalActivaId, contextMode, contextVersion]);
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-[#191a1d]">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="flex items-center gap-2 font-bold"><WalletCards size={19}/>Cajas</h2><p className="mt-1 text-sm text-slate-500">Catálogo operativo; no incluye saldos ni movimientos.</p></div>
      <div className="flex gap-2">{platform && <select aria-label="Filtrar por sucursal" value={filter || ''} onChange={e => setFilter(Number(e.target.value) || undefined)} className="rounded-xl border bg-transparent px-3 dark:border-slate-700"><option value="">Todas las sucursales</option>{sucursales.map(s => <option key={s.id} value={s.id}>{s.nombre}</option>)}</select>}<button onClick={create} disabled={readOnlyConsolidated} title={readOnlyConsolidated ? 'Seleccione una sucursal para crear una caja' : undefined} className="flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"><Plus size={16}/>Nueva caja</button></div>
    </div>
    {readOnlyConsolidated && <div className="mt-4 rounded-xl bg-blue-50 p-3 text-sm text-blue-700">Vista consolidada de solo consulta. Seleccione una sucursal para administrar cajas.</div>}
