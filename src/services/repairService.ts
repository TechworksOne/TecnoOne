...
const normalizeAccesoTipo = (value?: string | null): 'ninguno' | 'pin' | 'patron' => {
...
const getAccesoValor = (rep: any): string | null => {
  const value =
    rep.acceso_valor ??
    rep.accesoValor ??
    rep.contrasena ??
    rep.contraseña ??
    rep.patron_contrasena ??
    rep.patronContrasena ??
    rep.patronContraseña ??
    null;
...
const looksLikePattern = (value?: string | null): boolean => {
  const clean = String(value || '').trim();
  if (!clean) return false;

  return /^[1-9](?:[-,\s→]*[1-9]){1,8}$/.test(clean) && /[-,\s→]/.test(clean);
};

// ========== CREAR REPARACIÓN ==========
export const createReparacion = async (
  ...
  try {
    ...
    const recepcionAny = repairData.recepcion as any;
    ...
    const payload = {
      ...
      // Acceso
      patronContrasena:
        recepcionAny.patronContrasena ??
        recepcionAny.patronContraseña ??
        repairData.recepcion.accesoValor ??
        null,
      ...
      precioRevisionContrato: (repairData as any).precioRevisionContrato ?? null,
      condicionesServicioContrato: (repairData as any).condicionesServicioContrato ?? null,

      // Compatibilidad backend snake_case
      precio_revision_contrato: (repairData as any).precioRevisionContrato ?? null,
      condiciones_servicio_contrato: (repairData as any).condicionesServicioContrato ?? null,
...
export const changeRepairState = async (
  ...
  try {
    ...
    if (stateChange.subEtapa) {
      formData.append('subEtapa', stateChange.subEtapa);
    }

    formData.append('nota', stateChange.nota);

    if (stateChange.piezaNecesaria) {
      formData.append('piezaNecesaria', stateChange.piezaNecesaria);
    }

    if (stateChange.proveedor) {
      formData.append('proveedor', stateChange.proveedor);
    }

    if (stateChange.costoRepuesto) {
      formData.append('costoRepuesto', String(stateChange.costoRepuesto));
    }

    if (stateChange.stickerNumero) {
      formData.append('stickerNumero', stateChange.stickerNumero);
    }

    if (stateChange.stickerUbicacion) {
      formData.append('stickerUbicacion', stateChange.stickerUbicacion);
    }
